...
from app.inngest.operations import _validate_oauth_payload, _fetch_google_source_events_from_oauth, _fetch_google_source_events_from_mock
from app.inngest.pipeline import _run_pipeline_core, _run_post_ingest_steps, _build_pipeline_summary
from app.inngest.layers import _layer_ingest_and_process, _mark_run_succeeded, _mark_run_failed
...
async def replay_canonicalization(ctx: inngest.Context) -> dict[str, Any]:
    ...
    run_info = await ctx.step.run(
        ...
        _ensure_pipeline_run, data, "pipeline/stage.canonicalization.replay.requested", [],
    )
