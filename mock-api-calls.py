...
BASE_URL = (
    "https://kue-intelligence-tuyb.vercel.app/v1/ingestion/google/oauth/callback/mock"
)
...
for p in people:
    upd = (
        (now - timedelta(days=random.randint(0, 180), minutes=random.randint(0, 1440)))
        .isoformat()
        .replace("+00:00", "Z")
    )
    contacts_payload["payload"]["connections"].append(
        {
            ...
            "names": [
                {"displayName": p["name"], "givenName": p["fn"], "familyName": p["ln"]}
            ],
            "emailAddresses": [{"value": p["email"]}],
            "organizations": [{"name": p["company"], "title": p["title"]}],
            "metadata": {
                "sources": [
                    {"type": "CONTACT", "id": f"src_{p['id']}", "updateTime": upd}
                ]
            },
        }
    )
    ...
    calendar_payload["payload"]["items"].append(
        {
            ...
            "attendees": [
                {"email": x["email"], "responseStatus": "accepted"} for x in attendees
            ],
...
print("Posting contacts...")
post(contacts_payload)
time.sleep(1)
...
print("Posting calendar...")
post(calendar_payload)
